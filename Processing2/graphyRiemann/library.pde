...
abstract class Function {
  ...
  void drawRiemannRectangles(float left, float right, int numIntervals, boolean leftHanded) {
    drawFunctionOnInterval(X_MIN, X_MAX);
    float rectWidth = (right - left) / numIntervals;
    float rectScreenWidth = (float) (rectWidth  /(X_MAX - X_MIN) * width);
    fill(192, 64);
    for (int interval = 0; interval < numIntervals; ++interval) {
      float x = left + interval * rectWidth;
      double y = leftHanded ? value(x) : value(x + rectWidth);
      float yScreenHeight = (float) (y  /(Y_MAX - Y_MIN) * height);
      rect(xToScreenX(x), yToScreenY(y), rectScreenWidth, yScreenHeight);
    }
  }
...
class RiemannGraph {
  ...
  void setIntegrationRange(float x0, float x1, int numIntervals, boolean leftHanded_) {
    xLeft = x0;
    xRight = x1;
    numRectangles = numIntervals;
    leftHanded = leftHanded_;
  }

  void draw() {
    function.drawRiemannRectangles(xLeft, xRight, numRectangles, leftHanded);
    drawGridLines();
    if (SHOW_LABELS) {
      drawGridLabels();
    }
