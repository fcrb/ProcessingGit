...
void drawEllipses2(float strkWeight, float distanceToEllipseCenter, 
float ellipseRotation, float ellipseWidth, float heightFactor, int n)
{
  translate(width/2, height/2);
  noFill();
  strokeWeight(strkWeight);
  float sizeMultiplier = 1.49;// ((float)0.5) / (ellipseWidth + distanceToEllipseCenter);
  ...
  for (int i = 0; i < n; ++i) {
    float xDiameter = ellipseWidth * width * sizeMultiplier;
    float yDiameter = heightFactor * height * sizeMultiplier;
