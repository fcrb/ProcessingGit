...
class Maze {
  ...
  void createCircularGrid() {
    for (int i = 0; i < mazeRowCount; ++i) {
      for (int j = 0; j < mazeRowCount; ++j) {
        cellIsAvailable[i * mazeRowCount + j] = dist(i, j, (mazeRowCount - 1) * 0.5, (mazeRowCount - 1) * 0.5) <  (mazeRowCount * 0.5);
      }
    }
  ...
  int solutionLength() {
    ...
    while (currentLink != null && currentLink.c0 != startCell) {
      ++cellsInPath;
